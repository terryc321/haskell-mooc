...
sums :: Int -> [Int]
sums i = sum2 1 i 0
  where sum2 i lim acc = if i == lim then [acc + i]
                         else (acc +i) : sum2 (i + 1) lim (acc+i)
...
indexDefault (h:t) 0 def = h
indexDefault (_ :t) i def = indexDefault t (i - 1) def
...
sumsOf xs = helper xs 0
  where helper [] acc = []
        helper (h :t) acc = (h + acc) : (helper t (acc + h))
...
merge (x : xt) (y : yt) =
  ...
  else if y < x then y : merge (x : xt) yt
  else x : y : merge xt yt
...
mymaximum bigger initial (h : t) = if bigger initial h then mymaximum bigger initial t
                                   else mymaximum bigger h t
